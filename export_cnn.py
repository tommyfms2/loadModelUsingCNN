...
parse.add_argument('--cusize','-s',type=int, default=4)
...
print("W[0] : ", model.predictor.conv1.W.data[0][0][0][0])
print("W[1*64*3*3-1] : ", model.predictor.conv1.W.data[63][0][2][2])
print("b[0] : ", model.predictor.conv1.b.data[0])
print("b[64-1] : ", model.predictor.conv1.b.data[63])
...
print(c0,c1,c2,c3,b0)
print("W2[0] : ", model.predictor.conv2.W.data[0][0][0][0])
print("W2[64*128*3*3-1] : ", model.predictor.conv2.W.data[127][63][2][2])
print("b2[0] : ", model.predictor.conv2.b.data[0])
print("b2[128-1] : ", model.predictor.conv2.b.data[127])
...
print(c0,c1,c2,c3,b0)
print("W3[0] : ", model.predictor.conv3.W.data[0][0][0][0])
print("W3[128*256*3*3-1] : ", model.predictor.conv3.W.data[255][127][2][2])
print("b3[0] : ", model.predictor.conv3.b.data[0])
print("b3[256-1] : ", model.predictor.conv3.b.data[255])
print(c0,c1,c2,c3,b0)
...
open("linearonly.dat","w").write(l);

testDatas, correctLabels = load_test_data(args.cusize, args.path)
...
#    print(yx)
#print("done inimg")
#for yx in model.predictor.conv1.W.data[0][0]:
#    print(yx)
#print("done filter")

h = F.relu(model.predictor.conv1(x))
#print(len(h.data))
#print(len(h.data[0]))
#print(len(h.data[0][0]))
#print(len(h.data[0][0][0]))
#for a in h.data[0][0]:
#    print(a.reshape(-1))

h = F.relu(model.predictor.conv2(h.data))
h = F.relu(model.predictor.conv3(h.data))

bb = bytearray()
for v in h.data.reshape(len(h.data), -1):
    for v2 in v:
        bb += struct.pack('f',v2)
#print(len(h.data),len(h.data[0]),len(h.data[0][0]),len(h.data[0][0][0]))
open("conved.dat","w").write(bb);

h = F.relu(model.predictor.fc4(h.data))
#print(len(h.data))
#print(len(h.data[0]))
bbb = bytearray()
for v in h.data.reshape(len(h.data)*len(h.data[0])):
    bbb += struct.pack('f',v)
open("lineared.dat","w").write(bbb)

h = F.relu(model.predictor.fc5(h.data))
h = model.predictor.fc6(h.data)
#for i in np.arange(len(h.data)):
#    print(i,x[i])
#    print("  ---", np.argmax(h.data[i]),"---")
#    print("  ",h.data[i])
